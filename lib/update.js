...
const update = async () => {
  await Promise.all(
    config.apis.forEach(async (apiConfig) => {
      ...
      switch (apiConfig.type) {
        case 'hitokoto': {
          ...
          if (!hitokotos)
            throw new Error(`Can't find hitokotos for ${apiConfig.path}`);
        ...
        case 'media': {
          ...
          fileList.forEach((file) => {
            ...
            if (
              ...
              if (!result[category]) Object.assign(result, { [category]: [] });
        ...
        default: {
          if (!fileList)
            throw new Error(`Can't find resources for ${apiConfig.path}`);
