...
const hitokoto = (req, res, next) => {
  ...
  if (!iconv.encodingExists(encoding)) {
    ...
    logger.debug(
      `${req.method} ${req.baseUrl}${req.url} Unsupport encoding:${encoding}!`,
    );
  }
  ...
  Object.entries(resources[req.baseUrl]).forEach(([c, h]) => {
    if (
      category === 'all' ||
      (typeof category === 'string' ? category === c : category.includes(c))
    ) {
  ...
  if (hitokotos.length === 0) {
    ...
    logger.debug(
      `${req.method} ${req.baseUrl}${req.url} No matching hitokoto found!`,
    );
    return;
  }
  ...
  switch (format) {
    ...
    default:
      ...
      logger.debug(`Invalid format: ${format}!`);
  }
